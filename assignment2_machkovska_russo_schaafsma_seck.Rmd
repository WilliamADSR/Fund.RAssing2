...
Starting with only one predictor, to explore how the variables behave ina simple model. 
...
### ST_Slope
```{r}
model_stslope <- glm(HeartDisease ~ ST_Slope, 
              data = train_heart, 
              family = "binomial")
...
### ExerciseAngima
```{r}
model_angina <- glm(HeartDisease ~ ExerciseAngina, 
              data = train_heart, 
              family = "binomial")
...
Not working. I will check later how to fix it. 
```{r}
set.seed(42)
...
```{r}
c(fit1 = cross1$acc.cv, fit2 = cross2$acc.cv, fit3 = cross3$acc.cv)
```
