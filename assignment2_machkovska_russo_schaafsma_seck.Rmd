...
# Context 
The World Health Organization (WHO) estimated that 17.9 million people die each year from cardiovascular diseases (CVDs), globally. That is an estimated 32% of all deaths worldwide. Heart failure is most commonly caused by CVDs. Some people have inherited cardiac conditions but there are several behavioural factors that increase risk of heart failure: unhealthy diet, drug use, lack of exercise. Those behaviours can lead to individual raised cholesterol, blood pressure, etc. This dataset holds 11 variables that may predict heart failure, on which we will elaborate later on. In this assignment we will try to develop a model that overall, is accurate in predicting heart failure (or not) with new patients’ data. One could understand that such a model has a worldwide relevance.
...
# Research question 
As we already mentioned, our goal is to make accurate predictions for new patients’ data. Therefore we tend not to focus that much on inference. We will split our data into training data and testing data. We will build our model in the training data and then test our model / make predictions in the testing data. When interpreting output, we will work with a significance level of alpha 5% (p < 0.05) and a confidence interval (CI) of 95%.  

In this assignment we will do an exploratory analysis on the variables that could predict heart disease.  

Consequently, we will not work with hypothesis. Since we will try to build a model by exploratory means, our research question states:  

**When using exploratory data analysis, what variables seem to be influential predicting heart disease?**
...
We will work with a dataset that has 918 observations and 12 variables. We will inspect it, look for any anomaly and fix it in case we find it. After that, we will split the data in train and test sets. 
...
summary(heart)
```
This dataset contains information about 918 people of which 725 are males. What strikes is that resting blood pressure and cholesterol have some 0 values. What’s great is that our outcome variable “heart disease”, is nearly 50/50 for Norm and Prob. Finally, zero values in resting blood pressure and cholesterol may be missing values and looking at the interquartile range from max heart rate there may be some outliers between the min and 1st quartile.
Apparently, none of the variables have missing values, but we will get more insights in the descriptive analysis.
...
Now that we know what the variables consist of and the meaning behind them we will be looking some analysis on the distribution of the observations for each variable. 

```{r}
Hist_age <- hist(heart$Age, main = "Distribution of age", ylab = "Age in years")

Box_age <- boxplot(heart$Age, main = "Distribution of age", ylab = "Age in years")
```
...
Age is distributed normally. The youngest observations is 28 years old and the oldest observation is 77 years old. The median of this data set is 54 and there are no outliers.


```{r}
Hist_BPrest <- hist(heart$RestingBP, main = "Distribution of resting blood pressure levels", ylab = "Resting blood pressure levels (mmHg)")
```


```{r}
Box_BPrest <- boxplot(heart$RestingBP, main = "Distribution of resting blood pressure levels", ylab = "Resting blood pressure levels (mmHg)")
```
...
```{r}
Hist1_chol <- hist(heart$Cholesterol, main = "Distribution of cholesterol levels", ylab = "Cholesterol levels (mmHg)")
```

```{r}
Box_chol <- boxplot(heart$Age, main = "Distribution of cholesterol levels", ylab = "Cholesterol levels (mm / dl)")
```
...
Furthermore we see that the distribution of cholesterol levels has several outliers. The median is a cholesterol level of 237 which is within the range of a healthy level. a The lowest level of cholesterol is 85 and the highest level of cholesterol is 603. 

Due to 172 observations of the dataset (almost 20%) having the value of 0 for the variable cholesterol, it is not possible to simply remove those observations from the dataset. It would be better to entirely remove the variable then, but it is still yet uncertain whether cholesterol is related to heart disease. To determine that we need to see if cholestorol has any correlation to heart disease after removing the 0 observations.
...
From this we can determine that cholesterol has a very weak correlation. So to we have come to the decision to exclude the cholesterol variable.
...
```{r}
hist_MaxHR <- hist(heart$MaxHR, main = "Distribution of maximum heartrate", ylab = "Heartrate in bpm")

Box_maxhr <- boxplot(heart$MaxHR, main = "Distribution of maximum heartrate", ylab = "Heartrate in bpm")
...
Heartrate has a normal distribution and a couple outliers on the lower side, but nothing that doesn't fit the variable. The median maximum heart rate is 138 bpm which is average and within the norms for the median age of the observant. The highest is 202, which is definitely on the higher side and not healthy and the lowest is 60, which also isn't very healthy.

```{r}
hist_oldPeak <- hist(heart$Oldpeak, main = "Distribution of ST segment depression", ylab = "ST depression size")

Box_oldPeak <- boxplot(heart$Oldpeak, main = "Distribution of ST segment depression", ylab = "ST depression size")
...
Heartrate has a left-skewed distribution and a couple outliers on the higher side, but nothing that doesn't fit the variable. The median ST depression is  0.6 , the highest is 6.2 which is very high and caused by an outlier and the lowest is -2.6 which is quite low. The ST depression being to high or low can be indicators of anginaor other heart illnesses.

# Split the data: train and test
We will now split the data in two subsets, in order to train our model in the first set and check its performance in the second one. 

The train set will have 80% of the observations (735), and the test will be left with the other 20% (183 observations).  

Out of the 735 observations from the training set,  45% has normal heart performance (328 rows) and 55% has heart problems (407 rows). Therefore, the training data has approximately equal distribution of classes within response variable. 
```{r}
set.seed(42)
...
After the exploration made above, we concluded that *ExerciseAngina*, *ST_Slope*, *MaxHR*, *Oldpeak* and *Age* are the variables that show strongest correlation with our outcome variable, *HeartDisease*. Therefore, we are going to look for the best model from these variables. 
...
In order to assess whether this model provides a good fit to the data, we must check the residual plots. Furthermore the model must complete the assumptions for logistic regression namely; the outcome follows a binomial distribution, the predictor matrix is full-rank, the predictors are linearly related to *logit* and the observations are independent after accounting for the predictors.


The dataset has a binary outcome, which means that it will have a binomial distribution. This fulfills the first assumption.

```{r}
dim(dummy_heart)
```
The dimensions of the dataset are 735 rows by 18 columns, so the dataset also easily fulfills the second assumption of being full-rank.
...
Looking at the Residuals vs Fit plot, the data follows a linear trend. Residuals do seem to have a pattern, but that pattern of clusters is natural with a pearson plot. So this satisfies the assumptions that the predictors are linearly related.
...
plot(fit10,4)
```
According to the Cook's distance plot there are three cases which are potential influential outliers: 270, 407 and especially 564. The Residuals vs Leverage plot will provide more detail on these observations.
...
plot(fit10,5)
```
There are some cases which have high leverage such as 270 or 565 or have high residuals, but thankfully those cases don't have both. This means that there aren't any significant observations with both high residuals and leverage, so there are not highly influential cases and the observations are independent. 
...
After getting all these measures above, we can use ROC (Receiver Operating Characteristic) Curve to visualize the performance of our binary classifier.


The curve is plotted with the false positive rate on x-axis and true positive rate on y-axis for all possible thresholds. Both false positive rate  and true positive rate vary from 0 to 1.  

We will analyse the ROC curve in our test set to avoid bias and overfit that we could get if we would have analysed the curve using our training set. 

```{r}

# Duplicate HeartDisease column
test_heart_pred$prob_heart_obs <- test_heart_pred$HeartDisease    

# Transform the categorical column Pron and Norm in 0 and 1. Prob will be 1
test_heart_pred$prob_heart_obs <- ifelse(test_heart_pred$prob_heart_obs == "Prob", 1, 0)

# Use observed values against predicted values to make the roc curve
pred <- prediction(test_heart_pred$prediction, test_heart_pred$prob_heart_obs)
roc_curve <- performance(pred, measure = "tpr", x.measure = "fpr")
plot(roc_curve, colorize = T, lwd = 2)
abline(a = 0, b = 1)
```

The AUC (area under the curve) is a good way of interpreting the graph. As the name suggest, it is about calculating the area that is under the ROC curve -in our case, the area between the colorful line and the x-axis. A bad result would be close to 0.5, exactly where the diagonal line crosses the chart. This result would mean a random prediction, similar to flipping a coin. A good result is as close to 1 as possible, when the ROC curve would reach the left upper corner of the graph.  

As seen in the output bellow, the area under the curve in our chart is 0.913, meaning that we have a good classifier. 

```{r}
# Getting the values for the area under the curve
...
summary(fit10)
```
The reference groups are in this case *ExerciseAngina_N* and *ST_Slope_Up*. 

For an additional unit of *MaxHR*, the log odds of having a heart disease versus a normal performance decreases by **0.02**, after controlling for presence of an excercise angina, types of slopes and values of oldpeak. The effect is significant. The higher the maximum heart rate detected, the less chance the patient has of heart disease diagnosis. 

For an additional unit of *Oldpeak*, the the log odds of having a heart disease versus a normal performance significantly increases with **1.08**, after controlling for other predictors. The higher the values of oldpeak, the bigger chance there is for having a heart disease. 

Having an excersise induced angina (*ExerciseAngina_Y*) versus none (*ExerciseAngina_N*), changes the log odds of diagnosed heart by **1.41**, when the other predictors are at fixed values. 

For the groups within the predictor *ST_Slope*, patients with down slopes versus up slopes have an increase in log odds of probability of heart disease with **1.51**. For patients with flat slopes instead of up, the log odds of heart disease increase by **2.80**. 

In the presence of moderation of *ST_Slope* by *Oldpeak*, there is a different effect of oldpeak on heart disease probability for down and flat slope versus up slopes. This moderation works both ways. 
The coefficient for oldpeak is the effect of oldpeak when *ST_Slope_Down* = 0 and *ST_Slope_Flat* = 0. For *ST_Slope_Up*, a one unit increase in oldpeak has a change in log odds of **1.08**. On the other hand, for down slopes, a one unit increase in oldpeak changes the log odds with (1.077656 - 0.790886) = **0.29**. and for flat slopes log odds change with (1.077656 - 1.037129) = **0.04**, so the effect of oldpeak in having a heart disease is weaker for focal groups. 

Below are the coefficients expressed in odds ratio and given confidence interval of 95%.None of the confidence intervals have an odds of 1 in their range, so the results can be considered statistically significant. 
```{r, message=FALSE}
exp(cbind(OddsRatio = coef(fit10), confint(fit10)))
```

The odds for an extra unit of *MaxHR* are **0.98**, which is close to 1, so there is an almost negligible association between the maximum heart rate and the outcome. For an extra unit of *Oldpeak* the odds of success are **2.94**, so higher values of oldpeak are 3 times as likely to have a heart disease diagnosed. Holding MaxHR and Oldpeak at fixed values, the odds of having a heart disease for *ExerciseAngina_Y* over the odds of *EA_N* are 4.11, so *Y* group is 4 times more likely to have a heart disease diagnosed. 
Patients with *ST_Slope_Down* 4 times as likely to have heart disease and patients with *ST_Slope_Flat* are 16 times as likely compared to *ST_Slope_Up*. 
...
When oldpeak values are held at the mean, the patients with down and flat slopes are classified as having a normal heart performance for any value of MaxHR. For patients with up slopes, the overall probability of having a heart disease increases, when the values of maximum heart rate detected are lowered below 80. 
...
When oldpeak values are held at the mean, the patients with an exercise induced angina (yes) with MaxHR below 155 have a higher probability of having a heart disease. For no angina, the MaxHR detected above 80, means the lower probability of having a heart disease and less chance of being classified as problematic. 
...
When maximum heart rate values are held at the mean, the patients with down and flat slopes are classified as a normal heart for any value of oldpeak. For patients with up slopes, the probability of having a heart disease increases until it reaches a value of oldpeak of 2, when these patients are classified as having a heart disease in the model. 
...
When maximum heart rate values are held at the mean, the patients with exercise induced angina (yes), are classified as a problem case with lower values of oldpeak (around 0.2) compared to no angina (around 2). For both groups it is evident that the higher the oldpeak the higher the probability of having a heart disease. 

# Answering the research question 

The research question that we tried to answer in this assignment, stated: **“When using exploratory data analysis, what variables seem to be influential predicting heart disease?”**

Since the data set we used contained 11 variables influencing heart disease, we started by exploring correlations between the variables and heart disease. Considering the results from the correlation analysis we decided to try and build a best model with the following 5 variables: *ExcerciseAngina*, *ST_Slope*, *MaxHR*, *Oldpeak*, and *Age*.

After a series of building the model, from simple additive logistic regression modelling to moderated multiple logistic regression modelling, we ended up with our best model  Model 10. When executing the model comparisons this model came out best with regard to ANOVA, AIC and BIC. The results of our best model suggest that people that experience angina after exercise (ExerciseAngina_Y), have a ST slope that is downwards or flat (ST_Slope_Flat & ST_Slope_Flat) and have greater depression in the ST segment (Oldpeak), have an increased log odds of heart disease. Furthermore, people that have a higher maximum heart rate (MaxHR) and an interaction term for either a downwards or flat ST slope (ST_Slope_Flat & ST_Slope_Flat) and a depression in the ST segment (Oldpeak), have a decreased log odds of heart disease. Thus, the variables **ExcerciseAngina_Y**, **MaxHR**, **ST_Slope_Down**, **ST_Slope_Flat**, **Oldpeak**, **ST_Slope_Down x Oldpeak** and **ST_Slope_Flat x Oldpeak**, seem to be the main factors to predict whether someone has heart failure or not. 

Our goal when building this model was also to build a model that is accurate and shows low errors when predicting cases with new data. Our model has an accuracy of 85,79% and an AUC of 0,913. With regard to our outcome variable , *HeartDisease*, we are content with the way our model is able to do classifications. Since heart disease is a serious topic we would ultimately want a higher accuracy, yet the AUC is considered outstanding. 

Surprisingly, even though we did not include hypothesis in our study (as a result of exploratory analysis), *Age* was not a significant predictor of heart failure. One could also argue that *RestingBP* was unexpectedly not even moderately related to heart failure when doing the correlation analysis. Maybe another research is needed to investigate these relationships.

# Limitations

In our last topic, we will talk about the limitations faced during the development of this assignment. The first one to mention is our limitation to interpret the interaction added in our best model *fit10*. 

The addition of the interactions between *ST_Slope_Down* and *Oldpeak*, and *ST_Slope_Flat* and *Oldpeak* was worth due to the improvement in the performance of the model, but it was random and not based on any hypothesis about a moderation. Because of that, we are not allowed to say which of the variables (*ST_slope* and *Oldpeak*) is the moderator and which is the focal predictor. It would not be ethical to look for a hypothesis after finding an improvement in the model, so we will leave it for other researchers to dedicate themselves to reflect on the moderation between the two predictors.   

The second barrier was about the main predictors. As mentioned before, our goal was to train a model that could predict well and not find causal relationships or the indispensable variables that predict heart disease. We also did not base our analysis in any hypothesis about the true causes of heart problems. Because of that, we may have left some important variable out of the model. This could be the case, for instance, of cholesterol. We left it out, because there were a lot of missing values and a weak correlation with the outcome.
  
We also would like to mention the fact that the distribution of observations per class in categorical variables was not equal for some predictors. For the classification of logistic regression to be more accurate, the count per group should ideally be more balanced. However, that might correspond with the rareness of types occurring in real life medical cases.  Moreover, it is more important that the division in classes of the outcome group has a balanced number of observations. 

The last limitation we faced was a technical issue. When making the model comparison, we would like to add an extra criterion, besides the BIC, AIC and change in the deviance. We planned on performing a cross-validation through CVbinary() in order to get an estimate of accuracy, but because of and unknown problem, we were only able to get a zero result, as follows: 

Fold:  1 5 9 10 4 2 8 7 3 6  
Internal estimate of accuracy = 0  
Cross-validation estimate of accuracy = 0  

But it is here registered our attempt and concern about including this criterion, in addition to the code we would use to obtain this result.
...
# Comparing the models 3, 5, 6, 7, 8, 9, 10 and 11
...
Beerenwinkel, N., Ernst, F., Lengauer, T., Sander, O., Sing, T., & Unterthiner, T. (2020). *Package ‘ROCR’.* Retrieved from https://cran.r-project.org/web/packages/ROCR/ROCR.pdf 
...
Rousselet, G. A., & Wilcox, R. R. (2020). Reaction times and other skewed distributions: problems with the mean and the median. *Meta-Psychology, 4*. Retrieved from https://doi.org/10.15626/MP.2019.1630
