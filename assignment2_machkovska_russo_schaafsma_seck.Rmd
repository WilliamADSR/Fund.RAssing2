...
In order to assess whether this model provides a good fit to the data, we must check the residual plots. The coefficient estimated can turn out biased despite the relatively high Multiple
...
plot(fit10,5)
```
There are some cases which have high leverage such as 270 or 565 or have high residuals, but thankfully those cases don't have both. This means that there aren't any significant observations with both high residuals and leverage, so there are not highly influential cases. 
...
In the presence of moderation of *ST_Slope* by *Oldpeak*, there is a different effect of oldpeak on heart disease probability for down and flat slope versus up slopes. 
